    ...
    <main class="principal">
        ...
        <form action="<%= urlSubmit %>" method="post">
            ...
            <div>
                ...
                <textarea id="descricao" name="descricao" rows="4" required><%= (dlc != null ? dlc.getDescricao() : "") %></textarea>
            </div>
            ...
            <div>
                ...
                <select id="jogo_id" name="jogo_id" required>
                    ...
                    <%
                        if (jogos != null) {
                            for (Jogo j : jogos) {
                                boolean selecionado = (dlc != null && dlc.getJogoBase() != null
                                        && dlc.getJogoBase().getId() == j.getId());
                    %>
                        <option value="<%= j.getId() %>" <%= (selecionado ? "selected" : "") %>>
                            <%= j.getNome() %>
                        </option>
                    <%
                            }
